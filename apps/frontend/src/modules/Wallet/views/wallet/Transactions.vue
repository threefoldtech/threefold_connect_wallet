                ...
                <Listbox v-model="selectedAsset">
                    <div class="relative mt-1">
                        ...
                        >
                            ...
                            >
                                <ListboxOption
                                    v-for="assetCode in relevantAssetCodes"
                                    :key="assetCode"
...
<script lang="ts" setup>
    ...
    import { getOperationsInLocalStorage } from 'tf-stellar';
    ...
    const relevantAssetCodes = computed(() => {
        const values = walletBalance.value?.assets.map(a => a.name).filter(a => allowedAssets.indexOf(a) !== -1) || [];
        return ['ALL', ...values].filter((v, i, a) => a.indexOf(v) === i);
    });
    ...
    const computedOperations = computed(() => {
        ...
        return allOperations.operations.filter((operation: OperationRecord) => {
            return selectedAsset.value === 'ALL' || (<any>operation)?.asset_code === selectedAsset.value;
        });
    });
</script>
