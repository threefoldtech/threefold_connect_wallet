<template>
    <template>
        <TransitionRoot appear :show="true" as="template">
            <Dialog :initial-focus="completeButtonRef" as="div" @close="closeDialog">
...
<script lang="ts" setup>
    ...
    const saveNewContact = (contact: PkidContact) => {
        console.log(contact);
    };
    ...
    onMounted(async () => {
        ...
        const res = await pkidClient.getDoc(appKeyPair.value.publicKey, 'contacts');

        if (!res.success) {
            return;
        }

        pkidContacts.value = (await pkidClient.getDoc(appKeyPair.value.publicKey, 'contacts')).data.filter(
            (contact: Contact) => contact.type == chain
        );
    });
